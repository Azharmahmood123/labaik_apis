...
try {
    $stmt = $conn->prepare(" 
        ...
        FROM hadiths_shamail_muhammadiyah
        WHERE chapter_id = ?
    ...
    if (!$stmt) {
        jsonResponse(
            false,
            [],
            "Failed to prepare Shamail Muhammadiyah hadith query",
            500
        );
    }
    ...
    jsonResponse(
        ...
        message: "Successfully retrieved Shamail Muhammadiyah hadiths"
    );
    ...
    jsonResponse(
        false,
        [],
        "Failed to retrieve Shamail Muhammadiyah hadiths data",
        500
    );
}
