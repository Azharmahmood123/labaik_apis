...
try {
    $stmt = $conn->prepare(" 
        ...
        FROM hadiths_mishkat_almasabih
        WHERE chapter_id = ?
    ...
    if (!$stmt) {
        jsonResponse(
            false,
            [],
            "Failed to prepare Mishkat al-Masabih hadith query",
            500
        );
    }
    ...
    jsonResponse(
        ...
        message: "Successfully retrieved Mishkat al-Masabih hadiths"
    );
    ...
    jsonResponse(
        false,
        [],
        "Failed to retrieve Mishkat al-Masabih hadiths data",
        500
    );
}
